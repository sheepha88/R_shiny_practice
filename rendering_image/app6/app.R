...
library(shinyjs) #  make it easy to send a message to javascript
library(tools)
...
#getUploadTarget
getUploadTarget<-function(inputID){
  print(input[[inputID]])
...
server <- function(input, output, session){
  ...
  # modal 초기값 설정 -> 하나의 modal로 여러페이지에 쓰기 위해
  # modal에 값을 부여하여 switch문에서 해당되는 변수를 통해 조건문을 실행하게 한다.
  isolate({
...
shinyjs::runjs("$('#submit').hide();")
shinyjs::runjs("$('#submit').show();")
# henry
